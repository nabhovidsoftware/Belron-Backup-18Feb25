...
export default class lad_priceAdjustment extends NavigationMixin(LightningElement) {
    @api recordId;
    @api orderSummaryId
    ...
    conditionForSelection = ['Ordered', 'Intransit', 'Received at MDC', 'Backordered'];
    @track recordtypeId;
    @track options;
    isdataLoaded=false;
    ...
    get isButtonVisible(){
        return this.dataSet.every(i => this.conditionForSelection.includes(i.LAD_Status__c));
    ...
    connectedCallback() {
        this.orderSummaryId=this.recordId;
        console.log(81,this.recordId);
        console.log(82,this.orderSummaryId);
        ...
        this.dataSet = this.dataSet.map(i => {
            if (i.isDisabled != true && i.Id == event.target.dataset.id) {
                ...
                if(this.dataSet.filter(i=> i.isDisabled ==false).length==1){
                    this.selectAllVariant =  i.buttonVariant ;
                    this.selectAllName =  i.iconName;
                }
            }
            return i;
        })
    ...
    handleNonReturnable(event){
        this.dataSet = this.dataSet.map(i => {
            if (i.Id == event.target.dataset.id) {
                i.LAD_Non_Returnable_Flag__c = event.target.checked;
        ...
        this.dataSet.map(i => {
            console.log(201, i);
                ...
                wrapper.push({
                    ...
                    'nonReturnable': i.LAD_Non_Returnable_Flag__c,
                });
