...
import { ERROR_RANGE_UNDERFLOW, stringOnlyHasNumbers } from 'c/lad_commonNumberInput';
import { defaultRules, errorLabels, OUT_OF_STOCK_EVT, VALUE_CHANGED_EVT, VALIDITY_CHANGED_EVT } from './lad_constants';
import { outOfStock as outOfStockDefaultLabel } from './lad_labels';
...
export default class Lad_productQuantitySelector extends LightningElement {
    ...
    //To receive product Id from parent
    @api productId;

    //To receive suggested quantity from parent
    @api quantitySuggested
    ...
    connectedCallback() {
        if (this.quantitySuggested) {
            this.dispatchEvent(
                new CustomEvent(VALUE_CHANGED_EVT, {
                    bubbles: true,
                    composed: true,
                    detail: {
                        value: this.quantitySuggested,
                        lastValue: this.quantitySuggested,
